...
const clearList = function(){
    while(
        listElement.hasChildNodes()){ //пока есть дочерние элементы списка
...
function changeStatus(event){
    if(event.target.style.textDecoration ==="line-throungh"){
        event.target.style.textDecoration = "none"
    }
    else{
        event.target.style.textDecoration = "line-throungh"
    }
}
